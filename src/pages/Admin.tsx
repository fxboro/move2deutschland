...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    if (metaDescription) {
      metaDescription.setAttribute(
        "content",
        "Move2Deutschland Administration Panel for managing applicant pipelines and document verification.",
      );
    }
  ...
  const handleUpdateStatus = async (
    ...
    try {
      ...
      if (phone) {
        ...
        if (result.success) {
          toast.success(
            `Status updated to ${newStatus} and notification sent!`,
          );
        } else {
          toast.warning(
            `Status updated, but notification failed: ${result.error || "Unknown error"}`,
          );
        }
      } else {
        toast.info(
          `Status updated to ${newStatus}! (No phone number for notification)`,
        );
      }
    } catch (error) {
  ...
  const handleApproveDocument = async (
    ...
    try {
      ...
      if (phone) {
        ...
        if (result.success) {
          toast.success(
            `${docType.toUpperCase()} approved and notification sent!`,
          );
        } else {
          toast.warning(
            `${docType.toUpperCase()} approved, but notification failed.`,
          );
        }
      } else {
  ...
  const handleSaveNote = async () => {
    ...
    try {
      ...
      setUsers(
        users.map((u) =>
          u.id === notesModal.userId
            ? { ...u, adminNotes: notesModal.text }
            : u,
        ),
      );
      ...
      <aside className="w-full md:w-64 bg-slate-900 backdrop-blur-xl border-r border-white/10 text-white flex flex-col md:min-h-screen sticky top-0 z-20 shadow-2xl">
        <div className="p-6">
          ...
          <nav className="space-y-2">
            ...
            <button className="flex items-center gap-3 w-full px-4 py-3 text-slate-300 hover:bg-white/5 rounded-xl font-medium transition-colors opacity-50 cursor-not-allowed">
              <Bell size={20} />
              Notifications
            </button>
          </nav>
          ...
          {activeTab === "overview" && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              {[
                ...
                <div
                  key={i}
                  className="bg-white/60 backdrop-blur-xl p-6 rounded-2xl shadow-sm border border-white/50 "
                >
          ...
          {activeTab === "applicants" && (
            <>
              ...
              <div className="bg-white/60 backdrop-blur-xl p-5 rounded-2xl shadow-sm border border-white/50 mb-8 flex flex-wrap gap-4 items-center">
                <div className="flex items-center gap-2 text-slate-655 font-bold">
                  <Filter size={18} /> Filters:
                </div>
                <select
                  value={filterPriority}
                  onChange={(e) => setFilterPriority(e.target.value)}
                  className="border border-slate-200 rounded-xl px-4 py-2.5 outline-none focus:border-prussian-blue focus:ring-2 focus:ring-prussian-blue/20 bg-white/50 text-slate-800 font-medium"
                >
                ...
                <div className="bg-prussian-blue text-white p-4 rounded-2xl shadow-lg mb-6 flex flex-wrap items-center justify-between gap-4 animate-in fade-in slide-in-from-bottom-4">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    >
                      ...
                      <option value="pending" className="text-slate-800">
                        Pending
                      </option>
                      <option value="submitted" className="text-slate-800">
                        Submitted
                      </option>
                      <option value="reviewing" className="text-slate-800">
                        Reviewing
                      </option>
                      <option value="approved" className="text-slate-800">
                        Approved
                      </option>
                      <option value="rejected" className="text-slate-800">
                        Rejected
                      </option>
                    </select>
                    <button
                    ...
                    <tbody className="divide-y divide-slate-100 ">
                      {loading ? (
                        ...
                        filteredUsers.map((user) => (
                          <tr
                            key={user.id}
                            className={`hover:bg-white/50 transition-colors border-b border-slate-100/50 ${selectedUsers.includes(user.id) ? "bg-blue-50/50 " : ""}`}
                          >
                            <td className="p-5">
                              <input
                                type="checkbox"
                            ...
                            <td className="p-5">
                              <div className="font-bold text-prussian-blue text-lg">
                                {user.profile?.name || user.email || "Unknown"}
                              </div>
                              <div className="text-sm text-slate-500 ">
                                {user.email}
                              </div>
                              <div className="text-sm text-slate-500 mt-2 flex items-center gap-1.5 bg-white/50 inline-flex px-2 py-1 rounded-md border border-slate-100 ">
                                  ...
                                  <div className="flex flex-wrap gap-1 mt-1">
                                    {(user.profile.subjects || [])
                                      ...
                                      .map((sub: any, idx: number) => (
                                        <span
                                          key={idx}
                                          className={`text-[10px] px-1.5 py-0.5 rounded border font-bold ${["A1", "B2", "B3"].includes(sub.grade) ? "bg-green-50 text-green-700 border-green-200 " : "bg-slate-50 text-slate-600 border-slate-200 "}`}
                                          title={sub.name}
                                        >
                                          {sub.name
                                            .substring(0, 3)
                                ...
                                {["waec", "transcript", "passport"].map(
                                  (docType) => {
                                    ...
                                    return (
                                      <div
                                        key={docType}
                                        className="flex items-center justify-between gap-4 bg-white/80 p-3 rounded-xl border border-slate-100 shadow-sm"
                                      >
                                        <div className="flex items-center gap-2 text-sm">
                                          <span className="capitalize font-bold text-slate-700 w-20">
                                            {docType}
                                          </span>
                                          {docData.status === "verified" ? (
                                            <span className="text-green-600 flex items-center gap-1.5 text-xs font-bold bg-green-50 px-2 py-1 rounded-md border border-green-100 ">
                                              <CheckCircle size={14} /> Verified
                                            </span>
                                          ) : (
                                            <span className="text-yellow-600 flex items-center gap-1.5 text-xs font-bold bg-yellow-50 px-2 py-1 rounded-md border border-yellow-100 ">
