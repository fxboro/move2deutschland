...
export default function LeadQuestionnaire() {
  ...
  const handleSubmit = async () => {
    ...
    if (formData.academicStatus === "High School") {
      const highGradesCount = (formData.subjects || []).filter((sub) =>
    ...
    const highPriority =
      ["High", "Sponsor"].includes(formData.financialReadiness) &&
      hasHighGrades;
    ...
    try {
      ...
      if (accountExists) {
        ...
        localStorage.setItem(
          "move2deutschland_assessment_pending",
          JSON.stringify({
            formData,
        ...
        setTimeout(() => {
          setIsRedirecting(true);
          navigate(
            `/auth?redirect=dashboard&email=${encodeURIComponent(formData.email)}`,
          );
        }, 3000);
      }
    } catch (error) {
                  ...
                  {formData.highSchoolExam && (
                    <div className="space-y-4">
                      ...
                      <p className="text-xs text-slate-500 mb-3 -mt-3">
                        Enter at least 5 subjects with your grades.
                      </p>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full p-4 rounded-xl border focus:ring-2 outline-none transition-all font-sans ${
                        formData.cgpa &&
                        (parseFloat(formData.cgpa) < 1.0 ||
                          parseFloat(formData.cgpa) > 5.0)
                    ...
                    {formData.cgpa &&
                      (parseFloat(formData.cgpa) < 1.0 ||
                        ...
                        <p className="text-red-500 text-xs mt-2 font-medium">
                          Please enter a valid CGPA between 1.0 and 5.0
                        </p>
                      )}
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">
                      Bachelor's Program / Course Name
                    </label>
                    <input
                      type="text"
                      ...
                      onChange={(e) =>
                        updateForm("bachelorProgram", e.target.value)
                      }
        ...
        return (
          <div className="space-y-6 text-left">
            ...
            <p className="text-sm text-slate-500 mb-6 bg-slate-50 p-4 rounded-lg border border-slate-100">
              Germany requires international students to have a blocked account
              with{" "}
              <div className="relative inline-block group cursor-help">
                <span className="font-bold text-prussian-blue underline decoration-dotted decoration-slate-400">
                  €11,904
                </span>
                ...
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-3 bg-slate-800 text-white text-xs rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10 shadow-xl pointer-events-none">
                  This is the "Sperrkonto" (Blocked Account) requirement set by
                  the German government to ensure you can cover your living
            ...
            <div className="space-y-3">
              {[
                {
                  id: "High",
                  label: "I have the €11,904 ready (Personal/Family funds).",
                },
                {
                  id: "Sponsor",
                  label:
                    "I have a recognized sponsor in Germany (Verpflichtungserklärung).",
                },
                {
                  id: "Saving",
                  label: "I am actively saving towards this amount.",
                },
                {
                  id: "Loan",
                  label: "I plan to secure a student educational loan.",
                },
                {
                  id: "Scholarship",
                  label:
                    "I require a full scholarship (e.g., DAAD) to proceed.",
                },
              ].map((option) => (
                <button
                  key={option.id}
                ...
                >
                  <div
                    className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${formData.financialReadiness === option.id ? "border-prussian-blue" : "border-slate-300"}`}
                  >
                    {formData.financialReadiness === option.id && (
                      <div className="w-2.5 h-2.5 bg-prussian-blue rounded-full"></div>
                    )}
                  </div>
                  {option.label}
                </button>
            ...
            <div>
              ...
              !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email) ? (
                ...
                <p className="text-xs text-slate-400 mt-3">
                  We'll send your eligibility report and next steps to your
                  email.
                </p>
              )}
            </div>
          ...
          <div className="py-8 text-center space-y-6">
            ...
            {hasExistingAccount ? (
              <p className="text-slate-600 max-w-sm mx-auto">
                Your eligibility results have been sent to{" "}
                <strong>{formData.email}</strong>. Check your inbox for your
                detailed assessment report.
              </p>
            ) : (
              <p className="text-slate-600 max-w-sm mx-auto">
                You are a great fit for the German tuition-free program. Create
                your free account to unlock your full assessment report.
              </p>
            )}
    ...
    switch (step) {
      case 1: {
        if (formData.academicStatus === "Bachelor's") {
          ...
          const isCgpaValid =
            !isNaN(cgpaNum) && cgpaNum >= 1.0 && cgpaNum <= 5.0;
      ...
      case 2:
        return formData.fieldOfInterest !== "";
      case 3:
        return formData.financialReadiness !== "";
      case 4: {
  ...
  if (isAdmin) {
    return (
      <div className="bg-white/90 backdrop-blur-2xl rounded-2xl shadow-2xl [0_20px_50px_rgba(0,0,0,0.5)] p-8 border border-slate-100 text-center space-y-6 max-w-lg mx-auto transition-all">
        ...
        <div className="space-y-2">
          <h3 className="font-heading text-2xl font-bold text-prussian-blue ">
            Admin Access Restricted
          </h3>
          <p className="text-slate-600 text-sm leading-relaxed">
            You are currently signed in as an **Administrator**. Administrators
            are not permitted to submit candidate eligibility applications or
        ...
        <div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row gap-3 justify-center">
          <button
            ...
            className="bg-prussian-blue text-white font-bold py-3 px-6 rounded-xl hover:bg-prussian-blue/90 shadow-md transition-all text-sm cursor-pointer "
          >
            Go to Admin Panel
          </button>
        </div>
  ...
  return (
    <div className="bg-white/90 backdrop-blur-2xl rounded-2xl shadow-2xl [0_20px_50px_rgba(0,0,0,0.5)] p-5 md:p-8 border border-slate-100 relative overflow-hidden flex flex-col transition-all">
      {/* Progress Bar */}
      {step < 5 && (
        <div className="mb-8">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 font-medium transition-colors ${
              step === 1
                ...
                : "text-slate-500 hover:text-prussian-blue"
            }`}
          >
